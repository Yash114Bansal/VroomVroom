...
class PendingPaymentsListView(generics.ListAPIView):
    """
    Get List Of Pending Payements (Passenger).

    API Endpoint to View Unpaid Rides.
    """
    ...
    def get_queryset(self):
        ...
        queryset = (
            RideModel.objects.filter(passengers=user)
            .exclude(paid_by=user)
            .order_by("id")
        )
        return queryset
...
class PassengerPaymentRequestView(APIView):
    ...
    API Endpoint to Request to Record a Cash Payment.
    """
    ...
    def post(self, request, ride_id, *args, **kwargs):
        ...
        if ride.status == "upcoming":
            return Response(
                {"message": "Cannot request payment for upcoming ride."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if passenger not in ride.passengers.all():
            return Response(
                {"message": "Passenger is not part of this ride."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if PassengerPayment.objects.filter(ride=ride, passenger=passenger).exists():
            return Response(
                {"message": "Payment request already exists for this ride."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        PassengerPayment.objects.create(
            ride=ride, passenger=passenger, cash_payment_verified=False
        )
        ...
        send_push_notification.delay(
            ...
            body=r"Dear {name}, "
            + f"{passenger.name} has requested to record cash payment for ride.",
            users=user_details,
        )
        return Response(
            {"message": "Passenger payment request created successfully."},
...
class DriverPaymentRequestListView(generics.ListAPIView):
    """
    Get List Of Pending Payements (Driver).
    ...
    def get_queryset(self):
        ...
        queryset = PassengerPayment.objects.filter(
            ride__user=driver, cash_payment_verified=False
        ).order_by("id")
        return queryset
...
class DriverPaymentRequestApprovalView(APIView):
    ...
    API Endpoint to Record a Cash Payment.
    """
    ...
    def post(self, request, ride_id, *args, **kwargs):
        try:
            payment_request = PassengerPayment.objects.get(
                ride__id=ride_id, cash_payment_verified=False
            )
        ...
        send_push_notification.delay(
            ...
            body=r"Dear {name}, Your Payment Request has Been Approved!.",
            users=user_details,
        )
