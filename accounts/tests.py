...
class FCMTokenUpdateViewTestCase(TestCase):
    def setUp(self):
        self.user = UserProfile.objects.create(email='test@example.com',password="testpassword",email_verified=True,phone_verified=True)
        self.client = APIClient()

    def test_update_fcm_token_authenticated(self):
        self.client.force_authenticate(user=self.user)
        new_token = 'new_token'
        response = self.client.put(reverse('fcm_token_update'), {'fcm_token': new_token}, format='json')
...
class EmailSubscriptionUpdateViewTestCase(TestCase):
    ...
    def test_update_email_subscription_authenticated(self):
        ...
        response = self.client.put(reverse('email-subscription-update'), {'subscribed_to_email': new_subscription_status}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
